...
// Get playback position with a time offset (for DVR rewind)
// offsetSeconds: how many seconds back from "now" the user wants to watch
export function getSyncedPlaybackPositionWithOffset(
  ...
  const now = new Date();
  const rewindedTime = new Date(now.getTime() - offsetSeconds * 1000);

  const secondsSinceMidnight =
    rewindedTime.getUTCHours() * 3600 +
    rewindedTime.getUTCMinutes() * 60 +
    rewindedTime.getUTCSeconds();

  const totalPlaylistDuration = playlistLength * AVERAGE_SONG_DURATION;
  const positionInCycle = secondsSinceMidnight % totalPlaylistDuration;
  const videoIndex = Math.floor(positionInCycle / AVERAGE_SONG_DURATION) % playlistLength;
  const seekToSeconds = Math.floor(positionInCycle % AVERAGE_SONG_DURATION);
