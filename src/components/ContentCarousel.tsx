...
export function ContentCarousel({ title, items, variant = 'default', showAIBadge = false }: ContentCarouselProps) {
  ...
  return (
    <section className="relative py-4 sm:py-6 md:py-8">
      {/* Header */}
      <div className="container mx-auto px-3 sm:px-4 md:px-6 mb-3 sm:mb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h2 className="font-display font-bold text-lg sm:text-xl md:text-2xl">{title}</h2>
            {showAIBadge && (
      ...
      <div className="relative">
        ...
        <motion.div
          ...
          className="flex gap-3 sm:gap-4 overflow-x-auto scrollbar-hide px-3 sm:px-4 md:px-6 pb-4 snap-x snap-mandatory"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
