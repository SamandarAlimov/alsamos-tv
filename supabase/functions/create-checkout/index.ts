...
serve(async (req) => {
  ...
  try {
    ...
    if (!priceId || typeof priceId !== 'string') {
      return new Response(JSON.stringify({ error: "Valid Price ID is required" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 400,
      });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `${origin || ALLOWED_ORIGINS[0]}/subscription?success=true`,
      cancel_url: `${origin || ALLOWED_ORIGINS[0]}/subscription?canceled=true`,
    });
